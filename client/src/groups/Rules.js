      ...
      for (var rule of rulescopy){
        if (rule._id==id){
          ...
          if(!rule.approval.includes(auth.isAuthenticated().user._id)){
            ...
            console.log("sending down",rule,approval,this.state.group.level,this.state.group.groupsbelow,rule.sentdown,this.state.group.level)
...
for (let gr of lowergroupids){
  console.log("Lower GROUP",gr,rule._id)
...
render(props) {
  ...
  if (this.state.rules){
    ...
    rulescomponent=this.state.currentPageData.map(item => {
      ...
      console.log("LEVEL MATCH?",(this.state.group.level==item.level),this.state.group.level,item)
      return(
        <>
        <div className="rule">
